#!/bin/bash
# Rosee — ship optimized JPEGs from assets-src into public/images.
# Every shipped image must be <=500KB (a taste-profile floor).
cd "$(dirname "$0")" || exit 1
...
for p in sorted(glob.glob('public/images/*.jpg')):
    ...
    # Nocturne band: the pipeline floor is 15, the target 20-45. Brighter
    # than that is fine and preferred; only underexposure is a defect.
    # Aubade, but the subject's hour is BEFORE sunrise — genuinely dimmer
    # than Nipis's backlit window. 65 is the floor here; anything above that
    # is a correct dawn plate, not a murky one.
    flag = '  <-- TOO DARK' if mean < 65 else ''
    print(f'{os.path.basename(p):22s} luma {mean:6.1f}  {kb:4d}KB{flag}')
PY
