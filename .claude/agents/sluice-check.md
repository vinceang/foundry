---
name: sluice-check
description: Judges whether a Sluice site — Foundry's duration phase — is actually stunning or merely compliant, and whether the thing it asks you to operate rewards operating it. Use after the critique loop has cleared the floors, before shipping. Give it a site path and a filmstrip; a still is not enough here. It returns a verdict and the specific changes that would move it from good to memorable.
...
> **Would this win Awwwards Site of the Day, or is it a still page with a scroll handle bolted on?**
...
- **A filmstrip is mandatory here, not optional.** `node tools/shot-strip.mjs
  <url> <outPrefix> --mode action` with the interaction steps, plus frames shot
  deliberately at the signature's own offset. Evenly spaced frames will straddle
  the instrument and miss it — that has already happened on this collection. A
  static capture shows the fallback, and the best thing on the page reads as a
  hole. Without a filmstrip you cannot answer question 3 and must say so.
...
### 3. Does the thing you operate reward operating it?

This is the question this phase exists for, and it needs the filmstrip. If you
were given stills only, you cannot answer it — say so and stop, rather than
guessing from a single frame.

A Sluice site hands the visitor a control. Four things to check, in order:

**Does anything actually change?** Compare the first and last frames of the
instrument's sequence. If the difference is a fade, a slide, or a number
counting up, that is decoration with a scroll handle on it. Real state change
means the thing being shown is in a genuinely different condition — a different
temperature, a different depth of ash, a different tide, a different year.

**Does it teach you something a still could not?** The test: name one fact the
visitor knows after operating it that they could not have read from a caption.
If you cannot, the instrument is a picture with extra steps, and the honest fix
is to delete it and print the caption.

**Is it reversible, and does it read the same going up?** Scrub backwards
through the frames. A state that only makes sense on the way down is a trigger
wearing a controller's clothes, and this phase bans that outright.

**Can a person follow it?** The most common failure here is a sequence that
resolves faster than someone can read. If the change between two adjacent frames
of a normal scroll is too large to take in, the rate is wrong. Say so, and say
by how much.

A page that moves and teaches you nothing is worse than a still one, because it
cost the visitor time and gave nothing back.

### 3b. Could this belong to another site in the collection?

Both phases share tools, taste and a showcase, so check against Foundry sites as
well as Sluice ones. Open the sibling screenshots and put them next to these.
Look for shared devices — the same opening move, the same rail, the same
readout block, the same hairline meter with new labels.

Name any device you find on both. That device is a defect, not a family
resemblance. A relabelled instrument is the specific way this phase will fail.
